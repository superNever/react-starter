...
### React 元素类型的具体说明：
 ...
 大写的JSX标签类型指的是一个React组件，这些标签被编译为直接引用的命名变量，所以如果你使用了JSX`<Foo/>`，`Foo` 必须是已经在作用域当中可以调用了。

### 使用前先引入React
 既然JSX被编译后使用的是`React.createElement`方法,那么我们的 `React`库必须已经在我们的JSX代码的作用域中被引入了。
 例如，所有的imports导入在代码中都是必要的，虽然`React`和`CustomButton`不是直接从JavaScript中引用的：
```bash
...
### 使用点的方法调用JSX属性
 你也可以考虑在一个react组件中的JSX内部使用点的方法调用它的属性，如果你有一个单一的模块用来输出多个组件，使用这种方式会很便利。例如，如果`MyComponents.DatePicker`是一个组件，那么你可以直接在JSX中使用它：
...
<MyComponent message={'hello world'} />
```
 当你传了一个字符串常量，并且它的值是HTML-unescaped（非转义）。这两种JSX的语句也是相同的。
```bash
...
### Props的默认值是“True”
 如果你没有给prop传值，它默认的值是`true`.这两种JSX的语句也是相同：
```bash
...
```
当你正在创建一个通用的容器的时候扩展属性会非常有用。然而，这种写法也会让你的代码更加凌乱，因为对于组件它更容易传递很多组件并不关心的props。所以我们建议你尽量减少这种语法的使用。
### JSX的子元素
...
```
一个React组件不能返回（return）多个React元素，但是一个JSX语句是能有多个子元素的，所以如果你想要一个组件渲染多个元素，你可以像这样把它用一个`div`包起来。
### JavaScrit语句
...
### 作为子元素的函数
通常，我们会将JavaScript语句插入到JSX中来求其中的一个字符串，一个React元素或者是一个列表中的值。然而，`props.children`在这当中像其他`prop`一样，它能够传递任何类型的数据，不仅仅是React知道的渲染类型。例如，如果你有一个自定义的组件，你可以使用`props.children`作为一个回调。
```bash
